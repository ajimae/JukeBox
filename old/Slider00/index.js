...
class App extends Component {
  ...
  angle = () =>
    getRelativeAngle((this.props.value / 100) * 360, this.props.initialAngle) ||
    this.state.angle ||
    this.props.initialAngle;

  thumbSelect = () => {
    addEventListener('touchmove', this.moveThumb)
  ...
  calculateAngle = (mouseX, mouseY) => {
    ...
    const angle =
      toDeg(Math.atan(y / x)) + (x < 0 ? 180 : 0) + (x >= 0 && y < 0 ? 360 : 0);
  ...
  limitAngleVariation = angle => {
    ...
    return nextRelativeAngle < currentRelativeAngle + this.limitAngleFactor &&
  ...
  calculateThumbPosition = angle => {
    ...
    const x = Math.cos(toRad(angle)) * (r + trackWidth / 2) + r + trackWidth;

    const y = -Math.sin(toRad(angle)) * (r + trackWidth / 2) + r + trackWidth;
  ...
  handleChange = angle => {
    const percent =
      (getRelativeAngle(angle, this.props.initialAngle) / 360) * 100;
    ...
    return (
      ...
      >
        <View
          ...
          ref={this.ref}>
          <Track width={this.props.trackWidth} color={this.props.trackColor} />
          <Arc
            r={this.props.r}
            angle={this.angle()}
